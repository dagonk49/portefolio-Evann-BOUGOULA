...
| **URL du site** | `https://evann-bougoula.dagz.fr` (fournie par Evann) : canonique, Open Graph, robots.txt, sitemap.xml, JSON-LD | Rien, sauf changement d'adresse | `src/lib/site.ts` ou variable de build `SITE_URL` |
| **Google Search Console** | Aucune validation visible (ni balise, ni TXT) | Au choix : **(1)** propriété « Domaine » `dagz.fr` → enregistrement TXT `google-site-verification=…` à ajouter dans la zone DNS de `dagz.fr` (chez l'hébergeur DNS réel : Cloudflare si les serveurs de noms sont ceux de Cloudflare, sinon Infomaniak) ; **(2)** propriété « Préfixe d'URL » `https://evann-bougoula.dagz.fr` → méthode « Balise HTML » : copier le code et reconstruire avec `GOOGLE_SITE_VERIFICATION=<code>`. Puis envoyer `https://evann-bougoula.dagz.fr/sitemap.xml` dans « Sitemaps » | Compte Google d'Evann, DNS, ou `compose.yaml` / variable de build |
| **Photo** | Pas de photo : JSON-LD `Person` sans `image`, aperçu de lien = carte texte | Photo professionnelle, si souhaitée | `src/lib/site.ts` (`homeJsonLd`) |
