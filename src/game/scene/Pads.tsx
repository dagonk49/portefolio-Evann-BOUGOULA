...
import { INTERACTABLES, ANOMALY_PLACEMENTS, type Interactable } from "../layout";
...
export function Pads({ reducedMotion }: { reducedMotion: boolean }) {
  return (
    <group>
      {INTERACTABLES.filter((i) => i.pad).map((item) => (
...
export function ActivePrompt() {
  ...
  const item = useMemo(() => availableInteractables(mission).find((i) => i.id === activeId) ?? null, [activeId, mission]);
  if (!item || panel || stabilizing) return null;
  ...
  const verb = isAnomaly ? (viewed ? "Consulter" : "Stabiliser") : "Interagir";
  ...
  return (
    <Html position={pos} center zIndexRange={[20, 10]} style={{ pointerEvents: "none" }}>
      <div className="pad-prompt" aria-hidden="true">
        <span className="pad-prompt__key">{coarse ? "Interagir" : "E · Entrée"}</span>
